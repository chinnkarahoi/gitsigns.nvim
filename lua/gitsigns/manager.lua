...
function M.apply_win_signs(bufnr, hunks, top, bot)
   ...
   for i, hunk in ipairs(hunks or {}) do
      if first_apply and i == 1 or
         top <= hunk.vend and bot >= hunk.start then
         signs.add(config, bufnr, gs_hunks.calc_signs(hunk))
...
local update0 = function(bufnr, bcache)
   ...
   if gs_hunks.compare_heads(bcache.hunks, old_hunks) then
      bcache.pending_hunks = util.copy_array(bcache.hunks)



      M.apply_win_signs(bufnr, bcache.pending_hunks)
